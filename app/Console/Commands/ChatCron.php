    ...
    public function handle()
    {
        ...
        foreach ($clients as $key => $client) {
            try {
                ...
                \App\Model\Request::where('source_type', 'own_server')->whereHas('requesthistory', function ($query) {
                        ...
                        foreach ($requests as $key => $request_data) {
                            try {
                                ...
                                if (strtotime($datenow) >= $next_hour_time) {
                                    ...
                                    if ($client->domain_name == 'curenik' && $request_data->service_id == 7 && $request_data->requesthistory->status == 'pending') {
                                        $next_hour_time = strtotime($request_data->booking_date) + 3600; // 1 hrs
                                        if (strtotime($datenow) >= $next_hour_time) {
                                            $request_data->requesthistory->status = 'failed';
                                            $request_data->requesthistory->save();
                                        }
