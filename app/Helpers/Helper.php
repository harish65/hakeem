...
class Helper{
    ...
    public static function sendAPNPushNotification($user,$params){
        ...
        if(Config::get('client_connected')){
            $name = Config::get("client_data")->domain_name;
        ...
        }else if(Config::get('client_connected') && Config::get("client_data")->domain_name=="mataki"){
            $pem_file_name = "matakii.pem";
        }else if(Config::get('client_connected') && Config::get("client_data")->domain_name=="elconsultation"){
            $pem_file_name = "elconsultation.pem";
        }else if(Config::get('client_connected') && Config::get("client_data")->domain_name=="hypnowsis"){
            $pem_file_name = "pemCert.pem";
        }else if(Config::get('client_connected') && Config::get("client_data")->domain_name=="hexalud"){
            $pem_file_name = "voipJorge.pem";
        }else if(Config::get('client_connected') && Config::get("client_data")->domain_name=="together"){
            $pem_file_name = "together.pem";
        }else if(Config::get('client_connected') && Config::get("client_data")->domain_name=="912consult"){
            $pem_file_name = "quilsonVOIP.pem";
        }else if(Config::get('client_connected') && Config::get("client_data")->domain_name=="petpal"){
            $pem_file_name = "petpal.pem";
        }else if(Config::get('client_connected') && Config::get("client_data")->domain_name=="hakeemcare"){
            $pem_file_name = "hakeemcare.pem";
        }
       ...
       if(Config::get('client_connected') && Config::get("client_data")->domain_name=="hexalud" || Config::get('client_connected') && Config::get("client_data")->domain_name=="912consult" ){
            $pemFile = $pem_file_name;
        }
        if(Config::get('client_connected') && Config::get("client_data")->domain_name=="telegreen" || Config::get('client_connected') && Config::get("client_data")->domain_name=="together"){
            $pemFile = $pem_file_name;
        }if(Config::get('client_connected') && Config::get("client_data")->domain_name=="petpal"  && Config::get('client_connected') && Config::get("client_data")->domain_name=="mataki" ){
            $pemFile = $pem_file_name;
        ...
        \Log::channel('custom')->info('APN Notification--------', ['notifications' => $notifications,'deviceTokens'=>$deviceTokens,'params'=>$params]);
        if(count($notifications)>0){
            $client = new PushClient($authProvider, $production = true);
            $client->addNotifications($notifications);
            $responses = $client->push();
            \Log::channel('custom')->info('APN Notification', ['responses'=>$responses,'notifications' => $notifications]);


            foreach ($responses as $response) {
                // The device token
                $response->getDeviceToken();
                \Log::channel('custom')->info('response->getDeviceToken()', ['responses'=>$responses,'notifications' => $response->getDeviceToken()]);
                // A canonical UUID that is the unique ID for the notification. E.g. 123e4567-e89b-12d3-a456-4266554400a0
                $response->getApnsId();
                \Log::channel('custom')->info('response->getApnsId()', ['responses'=>$responses,'notifications' => $response->getApnsId()]);

                // Status code. E.g. 200 (Success), 410 (The device token is no longer active for the topic.)
                $response->getStatusCode();
                \Log::channel('custom')->info('response->getStatusCode()', ['responses'=>$responses,'notifications' => $response->getStatusCode()]);
                // E.g. The device token is no longer active for the topic.
                $response->getReasonPhrase();
                \Log::channel('custom')->info('response->getReasonPhrase()', ['responses'=>$responses,'notifications' => $response->getReasonPhrase()]);
                // E.g. Unregistered
                $response->getErrorReason();
                \Log::channel('custom')->info('response->getErrorReason()', ['responses'=>$responses,'notifications' => $response->getErrorReason()]);
                // E.g. The device token is inactive for the specified topic.
                $response->getErrorDescription();
                \Log::channel('custom')->info('response->getErrorDescription()', ['responses'=>$responses,'notifications' => $response->getErrorDescription()]);
                $response->get410Timestamp();
                \Log::channel('custom')->info('response->get410Timestamp()', ['responses'=>$responses,'notifications' => $response->get410Timestamp()]);
            }
        }
    ...
    public static function sendAPNPushNotificationTest($params){
        ...
        if(count($notifications)>0){
            ...
            foreach ($responses as $response) {
                // The device token
                $response->getDeviceToken();
                var_dump(['getDeviceToken' => $response->getDeviceToken()]);
                // A canonical UUID that is the unique ID for the notification. E.g. 123e4567-e89b-12d3-a456-4266554400a0
                $response->getApnsId();
                var_dump(['getApnsId' => $response->getApnsId()]);
                ...
                // E.g. Unregistered
                $response->getErrorReason();
                var_dump(['getErrorReason' => $response->getErrorReason()]);
                // E.g. The device token is inactive for the specified topic.
                $response->getErrorDescription();
                var_dump(['getErrorDescription' => $response->getErrorDescription()]);
                $response->get410Timestamp();
                var_dump(['get410Timestamp' => $response->get410Timestamp()]);
            }
        }
    }
