...
class DataController extends Controller
{
    ...
    public static function appVersion(Request $request)
    {
        try {
            ...
            if (Config::get("client_connected") && Config::get("client_data")) {
                ...
                $data['domain_name'] = $client_data->domain_name;
                $data['gateway_key'] = $client_data->gateway_key;
                $data['razorpay_secret'] = $client_data->gateway_secret;
                $data['payment_type'] = $client_data->payment_type;
            }
    ...
    public function getClientDetail(Request $request)
    {
        ...
        if (Redis::exists('client_feature_' . $app_id.'_'.$user_type) && $cache_enable) {
            $data = json_decode(Redis::get('client_feature_' . $app_id.'_'.$user_type), true);
    ...
    public function getClientDetailFromDB($app_id, $user_type = '')
    {
        ...
        $data['currency_code'] = Config::get('constants.CURRENCY_CODE');
        $data['jitsi_id'] = 202001;
        $app_types = ['both'];
        if ($user_type) {
        ...
        if (isset($data['domain']) && $data['domain'] == 'mataki')  {
            $data['domain_url'] = "https://mataki.admin.mobarni-ts.com";
        }else{
            $data['domain_url'] = "https://".Config::get('constants.DOMAIN_URL');
        }
        $data['media_url'] = Config::get('constants.MEDIA_URL');
        $data['socket_url'] = Config::get('constants.SOCKET_URL');
        $data['jitsi_meet_url'] = Config::get('constants.JITSI_MEET_URL');
        ...
        if ($client) {
            ...
            if (isset($data['domain']) && $data['domain'] == 'mataki')  {
                ...
                $data['domain_url'] = "https://$client->domain_name.".Config::get('constants.DOMAIN_URL');
            }
                    ...
                    foreach ($client_feature->feature_values as $key_id => $value) {
                        ...
                        if ($featurekey) {
                            ...
                            if (strtolower($client_feature->feature->name) == 'al rajhi bank' && ($client->domain_name == 'homedoctor')) {
                                ...
                                $data[$featurekey->key_name] = $value;
                            }else if (strtolower($client_feature->feature->name) == 'telr' && ($client->domain_name == 'careworks')) {
                                $data[$featurekey->key_name] = $value;
                            }
        ...
        Redis::set('client_feature_' . $app_id.'_'.$user_type, json_encode($data, true));
        return $data;
    }
    ...
    public static function getHomePageData(Request $request)
    {
        try {
            ...
            $forums = \App\Feed::select('id', 'title', 'image', 'description', 'like', 'user_id', 'created_at', 'views', 'favorite')->where('type', 'forum')->orderBy('id', 'desc')->take(5)->get();
            foreach ($forums as $key => $feed) {
                $user_data = User::select(['id', 'name', 'email', 'phone', 'profile_image'])->with('profile')->where('id', $feed->user_id)->first();
                $feed->user_data = $user_data;
                // $feed->comment = \App\Model\FeedComment::where(['feed_id' => $feed->id])->get();
                if (Auth::guard('api')->check()) {
                    $feedfavorite = FeedFavorite::where([
                        "user_id" => Auth::guard('api')->user()->id,
                        "feed_id" => $feed->id,
            ...
            if (Auth::guard('api')->check()) {
                $favourite = \App\Model\DoctorFavourite::where(['user_id' => Auth::guard('api')->user()->id, 'favourite' => 1])->orderBy('id', 'desc')->get();
                foreach ($favourite as $favourites) {

                    $favourites->user_data = User::where([

                        "id" => $favourites->doctor_id,
    ...
    public function postCustomMasterFields(Request $request)
    {
        ...
        return response([
            'status' => "success",
            'statuscode' => 200, 'message' => __('Added...')
        ], 200);
    }
     ...
     * @SWG\Post(
     *     path="/delete-banner",
     *     description="delete Banner From Service Provider",
     * tags={"Banners"},
     *     security={
     *     {"Bearer": {}},
     *   },
    ...
    public function deleteBanner(Request $request)
    {
        try {
            $user = Auth::user();
            $input = $request->all();
            $rules = ['id' => 'required|exists:banners,id'];
            $validation = \Validator::make($input, $rules);
            if ($validation->fails()) {
                return response(array('status' => 'error', 'statuscode' => 400, 'message' =>
                $validation->getMessageBag()->first()), 400);
            ...
            $banner =  \App\Model\Banner::find($request->id)->delete();
    ...
    public function contactus(Request $request)
    {
        try {
            ...
            return response(['status' => true, 'statuscode' => 200, 'data' => $contacts, 'message' => 'Contactus created Successfully'], 200);
        } catch (Exception $ex) {
            return response(['status' => "error", 'statuscode' => 500, 'message' => $ex->getMessage()], 500);
        }
    }
     ...
     * )
     */
    public function getCategorySymptoms(Request $request)
    {
        $per_page = (isset($request->per_page) ? $request->per_page : 10);
        $input = $request->all();
        $rules = array(
        ...
        if (@$input['sub_category_id']) {
            $MasterPreferences = \App\Model\CategorySymptom::select('id', 'name')
            ->where('is_enable', '1')
            ->where('category_id', $input['sub_category_id'])
            ->orderBy('id', 'DESC')->get();
            ...
            $subcategories = \App\Model\Category::where('parent_id', $input['category_id'])->get()->pluck('id');
            $MasterPreferences = \App\Model\CategorySymptom::select('id', 'name')
            ->where('is_enable', '1')
    ...
    /**
     * @SWG\Post(
     *     path="/update-user-symptoms",
     *     description="Update User Symptoms",
     * tags={"Master Preferences"},
     *
     *   @SWG\Parameter(
     *         name="symptom_ids",
     ...
     * )
     */
    public function updateUserCategorySymptoms(Request $request)
    {
        $per_page = (isset($request->per_page) ? $request->per_page : 10);
        $input = $request->all();
        $rules = array(
            'symptom_ids' => 'sometimes'
        );
        ...
        if(isset($input['symptom_ids'])){
            $symptom_ids = explode(',', $input['symptom_ids']);
            \App\Model\UserCategorySymptom::where(['user_id'=>Auth::guard('api')->user()->id])->delete();
            foreach ($symptom_ids as $key => $symptom_id) {
                    $usermasterpreference = new \App\Model\UserCategorySymptom();
                    $usermasterpreference->category_symptom_id = $symptom_id;

                    $usermasterpreference->user_id = Auth::guard('api')->user()->id;
                    $usermasterpreference->save();
            }
        }

            $usercategorysymptoms =  \App\Model\UserCategorySymptom::select('category_symptom_id as symptom_id')->where(['user_id'=>Auth::guard('api')->user()->id])->get();
            return response([
                'status' => "success",
                'statuscode' => 200, 'message' => __('List...'),
                'data' => [
                    "symptoms" => $usercategorysymptoms,
                ]
            ], 200);
    }
}
