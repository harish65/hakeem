    ...
    public static function getNotificationList(Request $request) {
    	try{
            ...
            if($request->has('reminder')){
                $notifications=Notification::select('id','notification_type as pushType','message','module','read_status','created_at','sender_id as form_user','receiver_id as to_user','module_id')->where(['receiver_id'=> $user->id,'notification_type' => 'REMINDER'])
                ->orderBy('id', 'desc')->cursorPaginate($per_page);
            }
	    	if($notifications){
	    		foreach ($notifications as $key => $notification) {
                    ...
                    $notification->create_date = \Carbon\Carbon::parse($notification->created_at)->diffForHumans();
	    		}
	    	}
