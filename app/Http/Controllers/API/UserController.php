...
use App\Model\UserCertification;
...
class UserController extends Controller
{
    ...
    public static function simple_login(Request $request)
    {
        ...
        if (Auth::attempt(['email' => request('email'), 'password' => request('password')])) {
            ...
            return response([
                ...
                'message' => __('login successfully !'), 'data' => ($updateduser)
            ], 200);
        } else {
     ...
     * @SWG\Post(
     *     path="/login",
     *     description="Login with Google,Facebook,Email,Phone,Apple,Microsoft",
     * tags={"User Register & Login Section"},
     *  @SWG\Parameter(
     *         name="provider_type",
     *         in="query",
     *         type="string",
     *         description="e.g facebook,google,email,phone,apple,microsoft",
     *         required=true,
     *     ),
     *  @SWG\Parameter(
     *         name="provider_id",
        ...
        try {
            ...
            switch ($request['provider_type']) {
                ...
                case 'facebook':
                    ...
                    // $config = [
                    //     'client_id' => isset($facebook_keys['api_key']) ? $facebook_keys['api_key'] : config('services.facebook.client_id'),
                    //     'client_secret' => isset($facebook_keys['secret_key']) ? $facebook_keys['secret_key'] :config('services.facebook.client_secret'),
                    //     'redirect' =>  isset($facebook_keys['redirect_url']) ? $facebook_keys['redirect_url'] : config('services.facebook.redirect'),
                    // ];
                ...
                case 'email':
                    ...
                    if ($email_otp) {
                        if ($input['provider_verification'] !== '9999' && !$verify) {
                            return response(array('status' => "error", 'statuscode' => 400, 'message' => 'Invalid OTP'), 400);
                        }
                    }
                    ...
                    } elseif (Auth::attempt([$column_name => $request->provider_id, 'password' => $request->provider_verification])) {
                        $user = Auth::user();
                        if (isset($request->fcm_id) && !empty($request->fcm_id)) {
                            User::where('id', $user->id)
                                ->update(['fcm_id' => request('fcm_id')]);
                ...
                case 'microsoft':
                    ...
                    if ($socialaccount) {
                        ...
                        if ($user) {
                            $user->account_verified = $datenow;
                            if (isset($request->user_api_id)) {
                                $user->user_api_id = $request->user_api_id;
                            }
                            $user->save();
                            $user = $user->addUserRequiredDetail($user, $request->user_type);
                            if (\Config::get('client_connected') && \Config::get('client_data')->domain_name == 'nurselynx') {
            ...
            if ($driver) {
                ...
                if ($socialaccount) {
                    ...
                    if (Config::get('client_connected') && Config::get('client_data')->domain_name == 'curenik') {
                        User::where('email', '=', $email)->update(['isUserFirstTime' => 'false']);
                    }
                } else {
            ...
            if ($user && ($user->hasrole($request->user_type) || $single_app)) {
                ...
                $updateduser->user_api_detail = [];
                if ($updateduser->hasrole('service_provider') && \Config::get('client_connected') && \Config::get('client_data')->domain_name == 'care_connect_live' && $updateduser->provider_type == 'microsoft') {
                    $user_api_detail = Helper2::getProviderDetail($request->provider_verification);
                    if ($user_api_detail) {
                        $updateduser->user_api_detail = $user_api_detail;
                    }
                }
                return response([
                    'status' => "success", 'statuscode' => 200,
    ...
    public static function register(Request $request)
    {
        try {
            ...
            if (\Config::get('client_connected') && \Config::get('client_data')->domain_name == 'carextrahealth') {
                if ($input['account_type'] !== 'individual' && $input['account_type'] !== 'business' && $input['account_type'] !== 'health_care') {
                    return response(['status' => "error", 'statuscode' => 400, 'message' => __(' Please provide correct user account type')], 400);
                }           
            }
            $device_type = strtoupper($request->header('devicetype'));
            if ($device_type != 'IOS') {
                $device_type = 'ANDROID';
            }
            ...
            if ($user) {
                ...
                if (isset($input['invite_code'])) {
                    ...
                    if (\Config::get('client_connected') && \Config::get('client_data')->domain_name == 'curenik') {
                        ...
                        $rewards = new \App\Rewards;
                        $rewards->reference_by = $ref_by->id;
                        $rewards->used_by = $user->id;
                        $rewards->wallet_id = $wallet->id;
                        $rewards->save();
                    }
                }
                ...
                if (isset($request->is_privacy_accepted)) {
                    $user->is_privacy_accepted = 0;
                    if ($request->is_privacy_accepted == true || $request->is_privacy_accepted == 'true') {
                        $user->is_privacy_accepted = 1;
                    }
                }
                if (isset($request->is_consent_accepted)) {
                    $user->is_consent_accepted = 0;
                    if ($request->is_consent_accepted == true || $request->is_consent_accepted == 'true') {
                        $user->is_consent_accepted = 1;
                    }
                }
                if (\Config::get('client_connected') && \Config::get('client_data')->domain_name == 'carextrahealth') {
                ...
                if (\Config::get('client_connected') && \Config::get('client_data')->domain_name == 'carextrahealth') {
                    ...
                    if (isset($input['id_file'])) {
                        $profile->id_file = $input['id_file'];
                    }
                }
                ...
                if (isset($input['user_gender'])) {
                    $profile->gender = $input['user_gender'];
                }
        ...
        try {
            ...
            $user = Auth::user();
                ...
                if (\Config::get('client_connected') && \Config::get('client_data')->domain_name == 'carextrahealth') {
                    ...
                    if (isset($input['certification_category'])) {
                        $user_certification->certification_category = $input['certification_category'];
                    }
                    if (isset($input['certification_type'])) {
                        $user_certification->certification_type = $input['certification_type'];
                    }
                    if (isset($input['certification_name'])) {
                        $user_certification->certification_name = $input['certification_name'];
                    }
                    if (isset($input['certification_file'])) {
                        $user_certification->certification_file = $input['certification_file'];
                    }
                    if (isset($input['launch_day'])) {
                        $user_certification->launch_day = $input['launch_day'];
                    }
                    if (isset($input['no_launch_day'])) {
                        $user_certification->no_launch_day = $input['no_launch_day'];
                    }
                    if (isset($input['expiratory_day'])) {
                        $user_certification->expiratory_day = $input['expiratory_day'];
                    }
                    if (isset($input['no_expiratory_day'])) {
                        $user_certification->no_expiratory_day = $input['no_expiratory_day'];
                    }
            ...
            return response(array(
                'status' => 'success',
                'statuscode' => 200,
                'message' => 'User Certification Info Saved',
                'data' => $user_certification
            ), 200);
        } catch (Exception $ex) {
            return response(['status' => "error", 'statuscode' => 500, 'message' => $ex->getMessage()], 500);
        }
     ...
     *         in="query",
     *         type="string",
     *         description="npi_id for care connect connect",
     *         required=false,
     *     ),
     *     @SWG\Parameter(
     *         name="user_api_id",
     *         in="query",
     *         type="string",
     *         description="user_api_id for care connect connect",
     *         required=false,
     *     ),
     *    @SWG\Parameter(
     *      name="profile_image",
    ...
    public function profieUpdate(Request $request)
    {
        try {
            ...
            if (isset($input['user_api_id'])) {
                $user->user_api_id = $input['user_api_id'];
            }
            ...
            if ($user->hasrole('service_provider') && \Config::get('client_connected') && \Config::get('client_data')->domain_name == 'care_connect_live' && $user->account_step == 1 && $user->provider_type == 'microsoft') {
                $user->account_step = 6;
                if (\Config::get('client_connected') && \Config::get('client_data')->domain_name == 'care_connect_live' && $request->user_type == 'service_provider') {
                    $datenow = new DateTime("now", new DateTimeZone('UTC'));
                    $time = new DateTime($datenow);
            ...
            if (\Config::get('client_connected') && \Config::get('client_data')->domain_name == 'careworks') {
                //Delete pincodes
                $alldata = [];
                $delete_pincodes = UserPincode::where(['user_id' => $user->id])->delete();
                if (isset($input['pincodes'])) {
                    if (!is_array($input['pincodes'])) {
                        $input['pincodes'] = explode(',', $input['pincodes']);
                    }
                    if ($input['pincodes']) {
                        foreach ($input['pincodes'] as $_pincode) {
                            $userdata['user_id'] = $user->id;
                            $userdata['pincode_id'] = $_pincode;
                ...
                //Update pincodes
                $update_pincodes = UserPincode::insert($alldata);
            }
            $user->device_type = $device_type;
            $user->save();
            $userz = User::with('roles')->find($user->id);
    ...
    /**
     * @SWG\Post(
     *     path="/doctor-favourite",
     *     description="Doctor favourite",
     * tags={"Security Question"},
     *     security={
     *     {"Bearer": {}},
     *   },
    ...
    public function doctorFavourite(Request $request)
    {
        ...
        try {
            ...
            $docfavourite = \App\Model\DoctorFavourite::where(['doctor_id' => $request->doctor_id, 'user_id' => Auth::user()->id])->first();

            if (!$docfavourite) {

                $docfavourite = new \App\Model\DoctorFavourite;

                $docfavourite->doctor_id = $request->doctor_id;

                $docfavourite->user_id = Auth::user()->id;

                $docfavourite->favourite = $request->favourite;
            ...
            if ($request->hasfile('image')) {
                if ($image = $request->file('image')) {
                    ...
                    if (isset($request->type) && strtolower($request->type) == 'pdf') {
                        ...
                        $image_name = $filename;
                    } elseif (isset($request->type) && strtolower($request->type) == 'audio') {
                        $FileEnconded =  \File::get($request->image);
                        \Storage::disk('spaces')->put('audio/' . $filename, (string)$FileEnconded, 'public');
                        $image_name = $filename;
                    } elseif (isset($request->type) && strtolower($request->type) == 'video') {
                        $FileEnconded =  \File::get($request->image);
                        ...
                        $image_resize = Image::make($image->getRealPath());
                         ...
                         $image_resize = $image_resize->resize($image_resize->width(), $image_resize->height(), function ($constraint) {
                             $constraint->aspectRatio();
                         })->encode('jpg');
                        
                        //  Log::info('My message', ['image_width' => $image_resize->width()]);
                        //  Log::info('My message', ['image_height' => $image_resize->height()]);
                        \Storage::disk('spaces')->put('original/' .$filename, $image_resize,'public');


                        \Storage::disk('spaces')->put('thumbs/' . $filename, (string)$thumb, 'public');
                        \Storage::disk('spaces')->put('uploads/' . $filename, (string)$normal, 'public');
                        //  \Storage::disk('spaces')->put('original/' . $filename, (string)$_400x400, 'public');
                        \Storage::disk('spaces')->put('800x800/' . $filename, (string)$_800x800, 'public');
                        \Storage::disk('spaces')->put('400x400/' . $filename, (string)$_400x400, 'public');
                        // \Storage::disk('spaces')->put('original/' . $filename, (string)$big, 'public');
                        $image_name = $filename;
    ...
    public static function getUserProfile(Request $request)
    {
        try {
            $user = Auth::user();
            $userz = User::with('roles', 'pincodes')->find($user->id);
            $token = $user->createToken('consult_app')->accessToken;
            $userz->token = $token;
    ...
    public static function forgot_password(Request $request)
    {
        ...
        $from_email='no-reply@petpal.asia';
        $user = User::where('email', $request->email)->first();
        $keys = Helper::getClientFeatureKeys('SMTP', 'smtp detail');
        if (isset($keys['mailer']) && isset($keys['username']) && isset($keys['password'])) {
            $config = array(
                'driver'     => $keys['mailer'],
                'host'       => $keys['host'],
                'port'       => '587',
                //'from'       => array('address' => $keys['username'], 'name' => 'Consultant'),
                'from'       => array('address' =>$from_email, 'name' => 'Consultant'),
                'encryption' => 'tls',
        ...
        if (@$user) {
            ...
            if ($request->hasHeader('user-type')) {
                if ($user && ($user->hasrole($request->header('user-type')))) {
                    if (is_object($user)) {
            ...
            return response(array('status' => "error", 'statuscode' => 400, 'message' => "User not found."), 400);
        }
    }
    ...
    public function deleteUserProfile(){
        ...
        if($user){
            ...
            $user->delete();
            return response(["status" => "success", 'statuscode' => 200, 'message' => __('User Account Deleted Successfully !')], 200);
        }else{
            return response(["status" => "error", 'statuscode' => 500, 'message' => $e->getMessage()], 500);
        }
    }
    ...
    public function deleteUser(){
        ...
        if($user){
            ...
            return response(["status" => "success", 'statuscode' => 200, 'message' => __('Account Deleted Successfully !')], 200);
        }else{
            return response(["status" => "error", 'statuscode' => 500, 'message' => $e->getMessage()], 500);
        }
    }
    ...
    public function getReferenceByList(Request $request)
    {
        try {
            $reference_by_user = User::select(
                ...
                'email',
                'device_type'
            )->with('roles', 'wallet')->where('reference_by', Auth::user()->id)->get();
        ...
        try {
            ...
            if (isset($input['documents']) && is_array($input['documents'])) {
                ...
                foreach ($input['documents'] as $key => $document) {
                    $image = new \App\Model\HealthRecordImage();
                    $image->ht_record_id = $healthrecords;
                    $image->health_doc = $document['image'];
    ...
    public function getHealthRecordsList(Request $request)
    {
        try {
            ...
            if (isset($request->type)) {
                $healthRecordsList = HealthRecords::where(['user_id' => $user->id, 'records_value' => $request->type])->orderBy('id', 'asc')->cursorPaginate($per_page);
            } else {
                $healthRecordsList = HealthRecords::where('user_id', $user->id)->orderBy('id', 'asc')->cursorPaginate($per_page);
            }
            foreach ($healthRecordsList as $healthRecords) {
                $healthRecords->documents = \App\Model\HealthRecordImage::where('ht_record_id', $healthRecords->id)->orderBy('id', 'asc')->get();
            }
        ...
        try {
            ...
            if ($request->hasfile('image')) {
                if ($image = $request->file('image')) {
                    ...
                    $normal1 = \Image::make($image)->resize(
                        260,
                        260,
    ...
    public function destroy($id)
    {
        try {
            ...
            $healthRecordImage = HealthRecordImage::where('ht_record_id', $id)->delete();
    ...
    public function deletehtrecords(Request $request)
    {
        try {
            $validator = \Validator::make($request->all(), [
            ...
            $healthRecords = HealthRecords::findOrFail($request->id);
            $healthRecordImage = HealthRecordImage::where('ht_record_id', $request->id)->delete();
            $healthRecords->delete();
    ...
    //                             'message' => __('User Profile Deleted...')], 200);
    //         }catch(Exception $ex){
    //             return response(['status' => "error", 'statuscode' => 500, 'message' => $ex->getMessage().' '.$ex->getLine()], 500);
    //         }
    // }
