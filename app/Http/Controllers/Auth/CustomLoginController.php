...
class CustomLoginController extends Controller
{
   ...
   /*--------------------------------- For iEdu---------------------------- */ 
    ...
    public function customSigup(Request $request){
        $request->validate([
    ...
    public function customSigupValidation(Request $request){
        $request->validate([
            'first_name' => 'required|string',
            'last_name' => 'required|string',
            'email'=> 'required|email|unique:users,email',
    ...
    public function customLogin(Request $request){   
        ...
        if($user->roles[0]['name']!==$input['role']){
            if($user->roles[0]['name']=='customer'){
                $current_role = 'Student';
            }else{
                $current_role = 'Teacher';
            }
            $errors['role'] = ["You are register as $current_role with same account, Please try with other account."];
            return response(['status' => "error", 'statuscode' => 400, 'errors' =>$errors], 400); 
        }
