...
class Transaction extends Model
{
    ...
    public static function createWithdrawal($transaction_detail){
		...
		if($transaction_detail['status']=='succeeded'){
            ...
            if($transaction->amount>0){
                ...
                $notification->save();
                $notification->push_notification(array($transaction_detail['user']->id),array('pushType'=>'Booking Reserved','message'=>__('notification.booking_amount_de_text', ['amount' => $transaction->amount])));
            }else{
                $notification->message =__('notification.booking_amount_de_text2');
                $notification->save();
                $notification->push_notification(array($transaction_detail['user']->id),array('pushType'=>'Booking Reserved','message'=>__('notification.booking_amount_de_text2')));
            }
    ...
    public static function createWithdrawalExtraPayment($transaction_detail){
        ...
        if($transaction_detail['status']=='succeeded'){
            ...
            $notification->save();
            $notification->push_notification(array($transaction_detail['user']->id),array('pushType'=>'Balance Deducted','message'=>__("$transaction->amount transferred to $sent_to->name for extra payment")));
            return $transaction;
        }
    }
    ...
    public static function createWithdrawalNew($transaction_detail){
        ...
        if($transaction_detail['status']=='succeeded'){
            ...
            $notification->save();
            $notification->push_notification(array($transaction_detail['user']->id),array('pushType'=>'Asked Question','message'=>__('notification.asking_question_de_text', ['amount' => $transaction->amount])));
        }
        return $transaction;
    }

    public static function createWithdrawalFromSPEmergency($transaction_detail){
        $transaction = self::create(array(
                'amount'=>$transaction_detail['balance'],
                'transaction_type'=>'withdrawal',
                'status'=>'pending',
        ...
        if($transaction_detail['status']=='succeeded'){
            ...
            $notification->message =__('notification.booking_amount_de_text3', ['amount' => $transaction->amount]);
            $notification->save();
            $notification->push_notification(array($transaction_detail['sp']->id),array('pushType'=>'Emergency','message'=>__('notification.booking_amount_de_text3', ['amount' => $transaction->amount])));
        }elseif($transaction_detail['status']=='payout-pending'){
            $transaction->status = 'inprogress';
            $transaction->save();
            $payment = \App\Model\Payment::create(array('from'=>$transaction_detail['from_id'],'to'=>$transaction_detail['sp']->id,'transaction_id'=>$transaction->id));
    ...
    public static function createWithdrawalFromSP($transaction_detail){
        ...
        if($transaction_detail['status']=='succeeded'){
            ...
            $notification->message =__('notification.booking_amount_de_text', ['amount' => $transaction->amount]);
            $notification->save();
            $notification->push_notification(array($transaction_detail['sp']->id),array('pushType'=>'Booking Reserved','message'=>__('notification.booking_amount_de_text', ['amount' => $transaction->amount])));
        }elseif($transaction_detail['status']=='payout-pending'){
            $transaction->status = 'inprogress';
            $transaction->save();
            $payment = \App\Model\Payment::create(array('from'=>$transaction_detail['from_id'],'to'=>$transaction_detail['sp']->id,'transaction_id'=>$transaction->id));
    ...
    public static function createRefund($transaction_detail){
        ...
        if($transaction_detail['status']=='succeeded'){
            ...
            $notification->message =__('notification.booking_amount_re_text', ['amount' => $transaction->amount,'user_name'=>$received_from->name]);
            $notification->save();
            $notification->push_notification(array($transaction_detail['user']->id),array('pushType'=>'Amount Received','message'=>__('notification.booking_amount_re_text', ['amount' => $transaction->amount,'user_name'=>$received_from->name])));
        }elseif($transaction_detail['status']=='vendor-pending'){
            $transaction->status = 'pending';
            $transaction->save();
        }else{
    ...
    public static function createRefundForStripe($request_data,$customer=false,$charges_full=true,$per_hour){
        ...
        $notification->save();
        $notification->push_notification(array($request_data->cus_info->id),array('pushType'=>'Amount Received','message'=>__('notification.booking_amount_refund_text', ['amount' => $payment,'user_name'=>$received_from->name])));
        return $transaction;
    }
    ...
    public static function createRefundForSP($transaction_detail){
        ...
        if($transaction_detail['status']=='succeeded'){
            ...
            $notification->message =__('notification.booking_amount_re_text', ['amount' => $transaction->amount,'user_name'=>$received_from->name]);
            $notification->save();
            $notification->push_notification(array($transaction_detail['sp']->id),array('pushType'=>'Amount Received',
                'message'=>__('notification.booking_amount_re_text', ['amount' => $transaction->amount,'user_name'=>$received_from->name])));
        }elseif($transaction_detail['status']=='vendor-pending'){
            $transaction->status = 'pending';
            $transaction->save();
    ...
    public static function updateDeposit($transaction_detail){
        ...
        if($transaction_detail['status']=='succeeded'){
            ...
            if(!$ignore_notification){
                $notification->push_notification(array($transaction_detail['user']->id),array('pushType'=>'Amount Received','message'=>__('notification.booking_amount_re_text', ['amount' => $amount,'user_name'=>$received_from->name])));
            }else{
