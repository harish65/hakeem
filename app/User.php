...
class User extends Authenticatable
{
     ...
     */
    protected $fillable = [
        'name','skills','education','address', 'email','user_name', 'password','phone','fcm_id','profile_image','email_verified_at','country_code','insurance_enable','manual_available','npi_id','source','profile_video','clinic_logo','signature','isUserFirstTime','police_verification','type',''
    ];

    protected $appends = ['name_hide','email_hide','phone_hide','total_rating_hide','review_count_hide','patient_count_hide'];


    public function getNameHideAttribute()
    {
        $res = false;
        $hide_inputs = EnableService::where('type', 'hide_inputs')->first();
        if($hide_inputs){
            $inputs = explode(',',$hide_inputs->value);
            $res = (in_array("name", $inputs))?true:false;
        }
        return $res;
    }
    ...
    public function getEmailHideAttribute()
    {
        ...
        if($hide_inputs){
            $inputs = explode(',',$hide_inputs->value);
            $res = (in_array("email", $inputs))?true:false;
        }
        return $res;
    }

    public function getPhoneHideAttribute()
    {
        $res = false;
        $hide_inputs = EnableService::where('type', 'hide_inputs')->first();
        if($hide_inputs){
            $inputs = explode(',',$hide_inputs->value);
            $res = (in_array("phone", $inputs))?true:false;
        }
        return $res;
    }

    public function getTotalRatingHideAttribute()
    {
        $res = false;
        $hide_inputs = EnableService::where('type', 'hide_inputs')->first();
        if($hide_inputs){
    ...
    public function getReviewCountHideAttribute()
    {
        $res = false;
        $hide_inputs = EnableService::where('type', 'hide_inputs')->first();
        if($hide_inputs){
            $inputs = explode(',',$hide_inputs->value);
            $res = (in_array("review_count", $inputs))?true:false;
        }
        return $res;
    }

    public function getPatientCountHideAttribute()
    {
        $res = false;
        $hide_inputs = EnableService::where('type', 'hide_inputs')->first();
        if($hide_inputs){
            $inputs = explode(',',$hide_inputs->value);
            $res = (in_array("patient_count", $inputs))?true:false;
        }
        return $res;
    }
    ...
    */
    ...
    {
        return $this->hasMany('App\Model\UserPincode', 'user_id');
    }
    ...
    */
    public function getServices($sp_id)
    {
        ...
        $st = implode("','",$st);
        $st = "'".$st."'";
        $categoryservicetype = CategoryServiceType::where('category_id', $cat->category_id)->orderByRaw("FIELD(service_id,$st)")->pluck('id')->toArray();
        if (count($categoryservicetype)<1) {
            return $subscriptions;
        }
        if(request('category_id') && request('date') && request('time') && request('end_time')){
            $services = CategoryServiceType::where('category_id', $cat->category_id)->pluck('service_id')->toArray();
            $input = [
                'sp_id'=>$sp_id,
            ...
            $services = Helper2::checkServiceAvailable($input);
            $categoryservicetype = CategoryServiceType::whereIn('service_id',$services)->where('category_id', $cat->category_id)->orderByRaw("FIELD(service_id,$st)")->pluck('id')->toArray();
        }
        $ct = implode("','",$categoryservicetype);
        $ct = "'".$ct."'";
        $subscriptions = \App\Model\SpServiceType::where([
            'sp_id'=>$sp_id,
            'available'=>'1'
        ])->whereIn('category_service_id', $categoryservicetype)->orderByRaw("FIELD(category_service_id,$ct)")->get();
        foreach ($subscriptions as $key => $subscription) {
            if ($subscription) {
    ...
    * @param
    */
    public static function getEnableServicesData($sp_id,$reqs)
    {
        $subscriptions = [];
        $services = [];
        $cat = \App\Model\CategoryServiceProvider::where('sp_id', $sp_id)->first();
        ...
        // $audio_calls = Service::where('service_type','audio_call')->pluck('id');
        // $video_calls = Service::where('service_type','video_call')->pluck('id');
        // $chats       = Service::where('service_type','chat')->pluck('id');
        // $now         = new Carbon();
        // $aYearAgo = $now->clone()->subYears(1);
        // $value =  Request::with('transanction','history')->whereIn('service_id',$audio_calls)->where('created_at','>=',$aYearAgo->format('Y-m-d H:i:s'))
        // ->where('created_at', '<=', $now->format('Y-m-d H:i:s'))
        // ->where('created_at', '<=', $now->format('Y-m-d H:i:s'))->get();
        // $video =  Request::with('transanction','history')->whereIn('service_id',$video_calls)->where('created_at','>=',$aYearAgo->format('Y-m-d H:i:s'))
        // ->where('created_at', '<=', $now->format('Y-m-d H:i:s'))
        // ->where('created_at', '<=', $now->format('Y-m-d H:i:s'))->get();
        // $chatt =  Request::with('transanction','history')->whereIn('service_id',$chats)->where('created_at','>=',$aYearAgo->format('Y-m-d H:i:s'))
        // ->where('created_at', '<=', $now->format('Y-m-d H:i:s'))
        // ->where('created_at', '<=', $now->format('Y-m-d H:i:s'))->get();
        ...
        //     $tra = Transaction::where(['request_id'=>$values->id,'status'=>'success','transaction_type'=>'deposit'])->first();
        //     if($tra){
        //         $au_amount = $au_amount + $tra->amount;
        //     }
        // }
        // foreach($video as $values){
        //     $tra = Transaction::where(['request_id'=>$values->id,'status'=>'success','transaction_type'=>'deposit'])->first();
        //     if($tra){
        //         $vu_amount = $vu_amount + $tra->amount;
        //     }
        // }
        // foreach($chatt as $values){
        //     $tra = Transaction::where(['request_id'=>$values->id,'status'=>'success','transaction_type'=>'deposit'])->first();
        //     if($tra){
        //         $chat = $chat + $tra->amount;
        //     }
        // }
        $rev = 0;
        $reve = 0;
        ...
        foreach($reqs as $req){
            if($req->requesthistory && $req->servicetype->service_type=='chat'){
                $rev = $rev +  $req->requesthistory->total_charges;
            }
            if($req->requesthistory && $req->servicetype->service_type=='audio_call'){
                $reve = $reve + $req->requesthistory->total_charges;
            }
            if($req->requesthistory && $req->servicetype->service_type=='video_call'){
                 $revenue =  $revenue + $req->requesthistory->total_charges;
            }
        }


        foreach ($subscriptions as $key => $subscription) {
            if ($subscription && $subscription->category_service_type->service->service_type == 'audio_call') {
                $services[] = array(
                ...
                // 'revenueChat'=>$chat,
                'revenue'=>$reve,
                // 'revenueVideo'=>$vu_amount,
               );
            }
            if ($subscription && $subscription->category_service_type->service->service_type == 'video_call') {
                $services[] = array(
                'service_name' => $subscription->category_service_type->service->type,
                'main_service_type' => $subscription->category_service_type->service->service_type,
                'service_id' => $subscription->category_service_type->service->id,
            ...
            if ($subscription && $subscription->category_service_type->service->service_type == 'chat') {
                $services[] = array(
                'service_name' => $subscription->category_service_type->service->type,
                'main_service_type' => $subscription->category_service_type->service->service_type,
                'service_id' => $subscription->category_service_type->service->id,
    ...
    */
    ...
    public static function getDoctorDetail($id)
    {
        ...
        if ($user && $user->hasRole('service_provider')) {
            ...
            $user->pincodes = $user->getPincodeData($user->id);
    ...
    */
        ...
        if (Config::get('client_connected') && Config::get("client_data")->domain_name=="iedu") {
            ...
            if ($cat) {
                ...
                if (\Config::get('client_connected') && \Config::get('client_data')->domain_name=='curenik') {
                    $category->additionals=$category->cure_additionals;
    ...
    public function getPincodeData($user_id)
    {
        $userpincodes = \App\Model\UserPincode::select('pincode_id')->where('user_id', $user_id)->get()->toArray();
        $pincodes = \App\Model\Pincode::whereIn('id', $userpincodes)->get();
        return $pincodes;
    }
