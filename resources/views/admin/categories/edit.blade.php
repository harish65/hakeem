...
<div class="container-fluid">
  ...
  <div class="row">
                  ...
                  <div class="input-group">
                    <div>
                      ...
                      <video width="200" height="200" controls>
                        <source src="{{ Storage::disk('spaces')->url('video/'.$category->video) }}" type="video/webm">
                        <source src="{{ Storage::disk('spaces')->url('video/'.$category->video) }}" type="video/mp4">
              ...
              <div class="form-group row">
                ...
                <div class="col-md-6">
                    ...
                    <select class="form-control show-tick" name="services">
                      <option <?php echo (old('services')=='Prescription') || $category->doctor_service == 'Prescription' ? "selected":'' ?> value="Prescription">Prescription</option>
                      <option <?php echo (old('services')=='Recommendation') || $category->doctor_service == 'Recommendation' ? "selected":'' ?> value="Recommendation">Recommendation</option>
                    </select>
                ...
                <div class="col-md-6">
                  <label for="name">Top Speciality</label>
                  <select class="form-control show-tick" required id="top_speciality" name="top_speciality">

                    <option <?php echo (old('top_speciality') == '0' || $category->top_speciality == '0') ? "selected" : '' ?> value="0">No</option>
                    <option <?php echo (old('top_speciality') == '1' || $category->top_speciality == '1') ? "selected" : '' ?> value="1">Yes</option>
                  </select>
                  @if ($errors->has('top_speciality'))
                  <span class="text-danger">{{ $errors->first('top_speciality') }}</span>
                  @endif
                </div>
                @endif
                ...
                <div class="col-md-6">
                  ...
                  <select class="form-control show-tick" required id="time_slot" name="time_slot[]">
                    <option {{ (in_array(1,explode(',',$category->time_slot)))?"selected":'' }} value="1">Hourly</option>
                    <option {{ (in_array(2,explode(',',$category->time_slot)))?"selected":'' }} value="2">Full Day</option>
                  </select>
                  @if ($errors->has('enable'))
                  <span class="text-danger">{{ $errors->first('enable') }}</span>
                  @endif
    ...
    @if(config('client_connected') && (Config::get("client_data")->domain_name=="curenik") && count($category->subcategory)==0)

    <div class="row">
      <div class="col-lg-6">
        <div class="card">
          <div class="card-body">
            <h4 class="mb-3 header-title">{{ __('text.Symptoms') }} <a href="{{ route('symptoms.create',$category->id) }}" class="btn btn-sm btn-info float-right">Add New</a></h4>
            <table id="scroll-horizontal-datatable3" class="table w-100 nowrap">
              <thead>
                <tr>
                  <th>Sr No.</th>
              ...
              <tbody>
                @foreach($category->symptoms as $index => $symptom)
                <tr>
                  <td>{{ $index+1 }}</td>
                  <td>{{ $symptom->name }}</td>
                  <td><?php echo ($symptom->is_enable == '1') ? "True" : 'False' ?> </td>
                  <td><a href="{{ route('symptoms.edit',[$category->id,$symptom->id]) }}" class="btn btn-sm btn-info float-left">Edit</a>
                    <!-- <button type="button" class="btn btn-danger btn-xs delete-filter"  data-filter_id="{{ $symptom->id }}">Delete</button> -->
                  </td>
                </tr>
                @endforeach
              </tbody>
