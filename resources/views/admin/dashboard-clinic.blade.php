              ...
              <div class="page-title-box mt-2">
                  <div class="page-title-right">
                      <ol class="breadcrumb">
						...
						<a href="{{ route('add_doctor')}}" class="btn btn-sm btn-info float-right mb-1 ml-2"><i class="fa fa-plus"></i> Add New Doctor</a>
                  </div>
		            ...
		            <tbody>
		             ...
		             @foreach($consultants as $index => $consultant)
			            <tr class="delete_row">
			              ...
			              <td><img src="{{ Storage::disk('spaces')->url('uploads/'.Auth::user()->profile_image) }}" height="50" width="50" id="frame" style="border-radius:100px;"  ></td>
			              @if(Config::get('client_connected') && (Config::get("client_data")->domain_name=="nurselynx"))
			              	<td id="uni_points_{{ $consultant->id }}">{{ $consultant->wallet->points }}</td>
			              @endif
			              <td>
							<a  title="Manage" href="{{ url('/service_provider/get_manage_availibilty') .'/'.$consultant->id}}" class="btn btn-sm btn-info "><i class="fa fa-tasks"></i> Manage Availability</a>
							
							<a  title="Edit Documents" href="{{ url('clinic/edit_upload_documnets') .'/'.$consultant->id}}" class="btn btn-sm btn-info "><i class="fe-plus"></i> Manage Documents</a>
						</td>
						<td><a  title="View" href="{{ route('bookings' , [ $consultant->id])}}" class="btn btn-sm btn-info "><i class="fe-phone"></i></a></td>
						<td>
                                ...
                                <div class="custom-control custom-switch" style="padding-top:5% !important">
                                    <input type="checkbox" class="custom-control-input toggleSwitch1" value="1" 
                                     id="toggleSwitch_{{ $consultant->id  }}" {{ ($consultant->	manual_available == 1 )  ? 'checked' : '' }}  data-user_id = "{{ $consultant->id }}">
                                    <label class="custom-control-label" for="toggleSwitch_{{ $consultant->id  }}">Select Availability</label>
                                </div>
		...
		$('#scroll-horizontal-datatable').on('click', '.deleteConsultant', function(e){
	          ...
	          Swal.fire({
	            ...
	            if (result.value) {
	                $.ajax({
	                   type:'POST',
	                   url:base_url+'/clinic/doctor_delete',
