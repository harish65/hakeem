@extends('layouts.vertical', ['title' => __('Doctor Manager')])
@section('css')
    <!-- Plugins css -->
	                ...
	                <div class="page-title-box">
	                    <div class="page-title-right">
	                        <ol class="breadcrumb m-0">
	                            ...
	                            <li class="breadcrumb-item active">{{ __('Doctor Manager') }}</li>
	                        </ol>
	                    </div>
	                </div>
            </div>
		    ...
		    <div class="col-12">
		      <div class="card">
		        <div class="card-header">
		          	<h3 class="card-title">{{ __('Doctor Manager') }}</h3>
		          	<a href="{{ url('admin/doctormanager/create')}}" class="btn btn-sm btn-info float-right">Add New {{ __('Doctor Manager') }}</a>
		        </div>
		        <!-- /.card-header -->
		        <div class="card-body">
		            ...
		            <tr >
		            	<th>Sr No.</th>
		            	<th>Action</th>
		            	<th>{{ __('Doctor Manager') }} Name</th>
		            	<th>Email</th>
		            	
		            </tr>
		            </thead>
		            <tbody>
		             @foreach($doctormanagers as $index => $manager)
		             	
			            <tr>
			              <td>{{ $index+1 }}</td>
			              <td>
			              	<ul style="padding: initial;">
			              		<li style="display:inline-block;"><a href="{{ url('admin/doctormanager') .'/'.$manager->id.'/edit'}}" class="btn btn-sm btn-info"><i class="fas fa-edit" style="cursor: pointer;"></i></a></li>
			              		<li style="display:inline-block;"><button data-user_id="{{ $manager->id }}" class="btn btn-sm btn-danger deleteCustomer"><i class="fe-trash"></i></button>
			              		</li>
			              		<li style="display:inline-block;"><button data-user_id="{{ $manager->id }}" data-user_name="{{ $manager->name }}" class="btn btn-sm btn-success openPasswordModal"><i class="fas fa-key"></i></button>
			              		</li>
					              	<a href="{{ url('admin/doctormanagers') .'/'.$manager->id}}" class="btn btn-sm btn-info "><i class="fe-eye"></i></a>
			              	</ul>
			              </td>
			              <td>{{ $manager->name }}</td>
			              <td>{{ $manager->email }}</td>
    ...
    <script type="text/javascript">
		...
		$('#resetPassword').on('click', function(e){
    		...
    		$('#scroll-horizontal-datatable').on('click', '.deleteCustomer', function(e){
		          ...
		          var user_id = $(this).attr('data-user_id');
		          Swal.fire({
		            title: 'Do You Want To Delete This Doctor Manager ?',
		            ...
		            if (result.value) {
		                $.ajax({
		                   type:'GET',
		                   url:base_url+'/admin/doctormanagers/delete-doctormanager',
		                   data:{"user_id":user_id},
		                   success:function(data){
		          			 _this.parents('tr').remove();
		                      Swal.fire(
		                        'Deleted!',
		                        'Doctor Manager has been deleted.',
		                        'success'
		                      ).then((result)=>{
