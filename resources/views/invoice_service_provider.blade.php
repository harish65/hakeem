		...
		<td>
			<table cellpadding="0" cellspacing="0" width="100%" style="width: 100%;">
				<tbody style="vertical-align: baseline;">
					<tr>
						<td>
							<h4 style="font-size: 22px;font-weight: 600;margin: 0 0 10px;">Curenik</h4>
							...
							<p style="margin: 12px 0 4px;color: #6b6b6b;">Dr. {{ $requesttable->sr_info->name }}</p>
							<p style="margin: 0 0 4px;color: #6b6b6b;">{{ $requesttable->sr_info->getCategoryData($requesttable->sr_info->id)->name }}:{{{ $requesttable->sr_info->country_code.''.$requesttable->sr_info->phone }}}</p>
							<p style="margin: 0 0 4px;color: #6b6b6b;">{{ $requesttable->clinic_info->name ?? ''}},{{ $requesttable->clinic_info->address ?? ''}}</p>
						</td>
						<td>
	...
	<tr>
		<td>
			<table cellpadding="0" cellspacing="0" width="100%" style="width: 100%;">
				...
				@php $price = $requesttable->transactions[0]->amount ?? 0;
				if($charges->key_name == 'percentage'){
					$admin_charges = $price * $charges->value/100;
				}else{
					$admin_charges = $charges->value;
				}
				
				$gst_charges = $admin_charges * 0.18;
				$total = $price - ($admin_charges+ $gst_charges);
				@endphp
					...
					<td style="vertical-align: baseline;">
						<table cellpadding="0" cellspacing="0" width="100%" style="width: 100%;">
							...
							<tr>
								...
								<td style="text-align: right;border: 1px solid #ddd;padding: 11px 6px;text-align: center;color: #717171; border: 1px solid #ddd;border-left:0;">- Rs.{{$gst_charges}}</td>
							</tr>
							<tr>
								<td style="border: 1px solid #ddd;padding: 11px 6px;text-align: center;color: #717171; border: 1px solid #ddd;border-left:0;">Curenik CSharges</td>
								<td style="text-align: right;border: 1px solid #ddd;padding: 11px 6px;text-align: center;color: #717171; border: 1px solid #ddd;border-left:0;">- Rs.{{$admin_charges}}</td>
							</tr>
							<!-- <tr>
							...
							<tr>
								...
								<td style="text-align: right;border: 1px solid #ddd;padding: 11px 6px;text-align: center;color: #717171; border: 1px solid #ddd;border-left:0;">Rs. {{$total}}</td>
							</tr>
						</table>
					</td>
				</tr>
