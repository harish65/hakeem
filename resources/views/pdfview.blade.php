...
<body style="">
	<div class="container-fluid">
		<div class="row">
			...
			<div class="col-6 full-mobile">
                ...
                @php
                        if(($requesttable->cus_info->profile && $requesttable->cus_info->profile->dob)){
                            $dob = $requesttable->cus_info->profile->dob;
                            $year = (explode("-",$dob));
                            $todayDate = \Carbon\Carbon::now()->format('Y');
                            $age =  $todayDate - $year[0];
                        }
                @endphp
				<p style="color: #777;">Age: {{$age ?? 'N/A' }}</p>
                {{-- change year to number in age  --}}
            ...
            @if(!empty($requesttable->profile) && !empty($requesttable->profile->PetBreed))
            <div class="col-12">

                <div class="col-6 full-mobile">
                    <h6 style="color: #000; margin-bottom: 8px; font-size: 15px; font-size: 22px;">Pet Information</h6>
                    <h3 style="color: #777; margin-bottom: 8px; font-size: 15px;">{{ $requesttable->profile->PetBreed->name }}</h3>
                    <p style="color: #777; margin-bottom: 8px; font-size: 15px;"> {{ $requesttable->profile->PetCategory->name }}</p>
                    {{-- <p style="color: #777; margin-bottom: 8px; font-size: 15px;">Email: {{ $requesttable->cus_info->email }}</p> --}}
                    @php
                            if(($requesttable->profile && $requesttable->cus_info->profile->dob)){
                                $dob = $requesttable->profile->dob;
                                $year = (explode("-",$dob));
                                $todayDate = \Carbon\Carbon::now()->format('Y');
                                $age =  $todayDate - $year[0];
                            }
			...
			<section class="table-section">
					...
					@foreach($requesttable->images as $img)
						<div style="height:auto;width:100%;overflow:hidden;padding:10px 0px 10px 0px;">
							<img src="{{ Storage::disk('spaces')->url('original/'.$img->image_name) }}" style="width:100%;height:auto;object-fit:cover !important;">
						</div>
					@endforeach
				</section>
			@endif
